...
authRouter.post("/ttcookie", function(req, res, next) {
    //console.log(req.body.cookie);
   var decoded = jwt_decode(req.body.cookie);
   console.log(decoded.data);
   //res.send(decoded.data);
   userS.findOne({'username': decoded.data.userId }, function(err, doc) {
        if (err) {
              console.log(err);
        }
        else {
             console.log(doc.amdin); 

              var checkpoint = {
                username : decoded.data.userId,
                check : doc.amdin
              }
              res.send(checkpoint);
